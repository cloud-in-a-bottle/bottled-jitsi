...
new = (
    ...
    "    #\n"
    "    # SECURITY: upstream jibri's template sets\n"
    "    #   java.util.logging.ConsoleHandler.level = FINE\n"
    "    # and jibri's metaconfig logger dumps EVERY resolved config\n"
    "    # value at FINE via ConfigSourceSupplier — including the\n"
    "    # JIBRI_XMPP_PASSWORD and JIBRI_RECORDER_PASSWORD. The console\n"
    "    # handler writes to stderr, which OpenHost surfaces as the\n"
    "    # owner-facing container log, so FINE leaks those secrets into\n"
    "    # the logs. Force the console handler to INFO; the FileHandler\n"
    "    # stays at FINE so full debug detail is still on disk under\n"
    "    # /config/logs-N/. (Do NOT raise ConsoleHandler back to FINE\n"
    "    # without redacting secrets first.)\n"
    "    tpl /defaults/jibri-logging.properties \\\n"
    "        | sed \"s#/config/logs/#/config/logs-${i}/#g\" \\\n"
    "        | sed \"s#^java.util.logging.ConsoleHandler.level = FINE#java.util.logging.ConsoleHandler.level = INFO#\" \\\n"
