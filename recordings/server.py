...
proxied to by nginx under /api/recordings/* and /recordings/*.
...
* ``/api/recordings/{init,chunk,finalize}`` — anonymous; anyone in a
  meeting can upload. The room name is a string the client provides
  and we store but don't trust.
* ``/recordings/<admin_token>/*`` — admin endpoints. The admin token
  is a 32-byte URL-safe random string generated on first boot and
  printed to the container logs. Possessing the token grants list /
  download / delete on every recording, indefinitely. There is no
  per-recording ACL — recordings live or die by whoever holds the
  admin URL. Treat the admin URL like a password.
...
def _total_finalized_bytes(rec_dir: Path) -> int:
    total = 0
    for meta in _list_meta(rec_dir):
...
def _evict_until_under_cap(rec_dir: Path, headroom_bytes: int, max_bytes: int) -> None:
    ...
    total = _total_finalized_bytes(rec_dir)
    for meta in metas:
        ...
        try:
            ...
            logger.warning("Eviction of %s failed: %s", rec_id, e)
            continue
        total -= size
...
def _sweep_orphans(rec_dir: Path) -> None:
    """At startup: drop any *.tmp files (interrupted uploads) and any
    metadata whose recording file is missing."""
    for p in rec_dir.glob("*.webm.tmp"):
        try:
            p.unlink()
            logger.info("Removed orphan tmp %s", p.name)
        except OSError as e:
            logger.warning("Could not remove orphan tmp %s: %s", p.name, e)
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _handle_finalize(self, rec_id: str) -> None:
        ...
        with _state_lock:
            ...
            meta["size_bytes"] = final.stat().st_size
            meta["finalized"] = True
            _write_meta(self.state.rec_dir, rec_id, meta)
